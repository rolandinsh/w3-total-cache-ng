...
<form action="admin.php?page=<?php echo $this->_page; ?>" method="post">
    <div class="metabox-holder">
        ...
        <table class="form-table">
            ...
            <?php endif; ?>
            <?php if ($this->_config->get_string('fragmentcache.engine') == 'memcache'): ?>
            <tr>
                <th><label for="memcache_servers"><?php w3_e_config_label('fragmentcache.memcache.servers') ?></label></th>
                <td>
                    <input id="memcache_servers" type="text"
                        <?php $this->sealing_disabled('fragmentcache') ?> name="fragmentcache.memcache.servers" value="<?php echo esc_attr(implode(',', $this->_config->get_array('fragmentcache.memcache.servers'))); ?>" size="100" />
                    <input id="memcache_test" class="button {nonce: '<?php echo wp_create_nonce('w3tc'); ?>'}" type="button" value="<?php _e('Test', 'w3-total-cache'); ?>" />
                    <span id="memcache_test_status" class="w3tc-status w3tc-process"></span>
                    <br /><span class="description"><?php _e('Multiple servers may be used and seperated by a comma; e.g. 192.168.1.100:11211, domain.com:22122', 'w3-total-cache'); ?></span>
                </td>
            </tr>
            <?php endif; ?>
            <?php if ($this->_config->get_string('fragmentcache.engine') == 'redis'): ?>
            <tr>
                <th><label for="redis_server"><?php w3_e_config_label('fragmentcache.redis.server') ?></label></th>
                <td>
                    <input id="redis_server" type="text"
                        <?php $this->sealing_disabled('fragmentcache') ?> name="fragmentcache.redis.server" value="<?php echo esc_attr($this->_config->get_string('fragmentcache.redis.server')); ?>" size="100" />
             ...
             <tr>
                <th><label for="redis_db"><?php w3_e_config_label('fragmentcache.redis.db') ?></label></th>
                <td>
                    <input id="redis_db" type="text"
                        name="fragmentcache.redis.db"
                        <?php $this->sealing_disabled('fragmentcache') ?>
                        value="<?php echo esc_attr($this->_config->get_integer('fragmentcache.redis.db')); ?>" size="100" />
